...
static ZenTreasure_Triggers GetZenTreasure_Triggers()
{
	...
	{
		Print("[ZenTreasure_Triggers] Init");
...
class ZenTreasure_Triggers extends Managed
{
	...
	void SpawnTrigger(ZenTreasureTriggerConfig cfg, PlayerBase player = NULL)
	{
		...
		if (!trigger)
		{
			Print("ZenTreasure_Triggers::SpawnTrigger() - Failed to spawn trigger!");
			return;
		}
		...
		Print("ZenTreasure_Triggers::SpawnTrigger() - Spawned trigger @ " + cfg.Position);
...
class ZenTreasure_StashTrigger extends ManTrigger
{
	...
	void SpawnTreasureLoot(notnull PlayerBase player, int stashType)
	{
		...
		Print("[ZenTreasure] " + player.GetCachedID() + " spawned treasure stash @ " + m_TreasureConfig.Position + " with config " + treasureConfig.ConfigName + " lootCount=" + lootCount);
	}
